	...
	public function legion_setting3_html() {
		...
		for($i=0;$i<sizeof($keys);$i++) {
			...
			echo '<div><input type="checkbox" id="legion_category" name="legion_menu['.$page.']" value="'.strip_tags($menuShow[$keys[$i]][0]).'"'.(isset($this->options['legion_menu'][$page]) ? " checked=\"checked\"" : '').' />'.(strpos($menuShow[$keys[$i]][2], "separator")!==false ? "Разделитель" : $menuShow[$keys[$i]][0]);
				if(isset($subMenuShow[$page])) {
					$skeys = array_keys($subMenuShow[$page]);
					for($z=0;$z<sizeof($subMenuShow[$page]);$z++) {
						$subpage = $subMenuShow[$page][$skeys[$z]][2];
						if(($pos = strpos($subpage, "dev"))!==false) {
							$subpage = substr($subpage, 0, $pos-1);
						}
						echo '<div><label for="'.$page."-".$subpage.'" class="submenu"><input type="checkbox" id="'.$page."-".$subpage.'" name="legion_submenu['.$page.']['.urlencode($subpage).']" value="'.strip_tags($subMenuShow[$page][$skeys[$z]][0]).'"'.(isset($this->options['legion_submenu'][$page][$subpage]) ? " checked=\"checked\"" : '').' />'.$subMenuShow[$page][$skeys[$z]][0]."</label></div>";
					}
				}
			echo "</div>";
